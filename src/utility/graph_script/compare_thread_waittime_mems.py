...
for logsize in logsizes:
    for op in ops:
        ...
        fig = plt.figure()
        plt.xticks(xtick, pmem_file.index)
        plt.bar(xtick-barwidth/2, pmem_file['End']       , width=barwidth, edgecolor = 'k', color = colorlst[1], label = '不揮発性メモリ')
        plt.bar(xtick+barwidth/2, vmem_file['End']       , width=barwidth, edgecolor = 'k', color = colorlst[2], label = 'DRAM')
        plt.xlabel('スレッド数', fontsize=fontsize)
