...
function make_csv () {
    ...
    do
        ...
        do
            ...
            acheck_time=`cat ${source_dir_tmp}/result_thr${thr}_trial${trial} | grep "sec. 3" | cut -f 6 -d ' '`
            fcheck_time=`cat ${source_dir_tmp}/result_thr${thr}_trial${trial} | grep "sec. 4" | cut -f 6 -d ' '`
        ...
        echo "$thr,$rcheck_time_avg,$ccheck_time_avg,$acheck_time_avg,$fcheck_time_avg" >> ${target_dir}/checkpointtime_${bench_type}.csv
    done
}
