...
def plot_graph(ax, plot_dir, bench_name, result_files, ledgends, colorlst, markerlst, font_size):
    result_dataframes = []
    ...
    for (df, color, marker) in zip(result_dataframes, colorlst, markerlst):
        df.plot(ax=ax, color=color, marker=marker, legend=False)
...
def main():
    ...
    for (bench_name, num) in zip(bench_names, range(len(bench_names))):
        result_files = list(map(result_file_generator, result_file_template))
        plot_graph(ax[int(num % 3), int(num / 3)], plot_target_dir, bench_name, result_files, ledgends, colorlst, markerlst, font_size)
    ...
    plt.savefig(plot_target_dir + '/all_threads_ja.pdf')
    plt.close()
