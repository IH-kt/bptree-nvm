...
def plot_graph(log_size_str, result_file1, result_file2, result_file3, result_file4, cols, memtype):
    ...
    for i in range(3):
        ...
        p_df1.columns = [result_file1.split('../' + memtype + '/compression/')[1]]
        p_df2.columns = [result_file2.split('../' + memtype + '/compression/')[1]]
        ...
        result_df = pd.concat([p_df1, p_df2], axis=1)
        # result_df.columns = ['col1', 'col2', 'col3', 'col4']
        result_df.columns = ['col1', 'col2']
        # result_df = p_df1
        ...
        plt.savefig(log_size_str + '/' + colname.split('_concurrent.exe')[0] + '_presen_ja.pdf')
        plt.close()
...
for memtype in ['pmem']: # ['vmem']: # ['pmem', 'vmem']:
    # result_file_dirs_1 = get_logsz_dir('../' + memtype + '/compression/bptree_nvhtm_0/')
    # result_file_dirs_2 = get_logsz_dir('../' + memtype + '/elapsed_time/bptree_nvhtm_1/')
    # result_file_dirs_2 = get_logsz_dir('../' + memtype + '/compression/bptree_nvhtm_0/')
    ...
    result_file_dir1 = '../' + memtype + '/compression/bptree_nvhtm_0/logsz_41943328/compressed/'
        # result_file_dir2 = '../' + memtype + '/elapsed_time/bptree_nvhtm_1/' + result_file_dirs_2[i]
    result_file_dir2 = '../' + memtype + '/compression/bptree_nvhtm_0/logsz_41943328/normal/'
        ...
        # plot_graph('scaling/' + memtype + '/' + result_file_dirs_1[i], result_file_dir1, result_file_dir2, result_file_dir3, result_file_dir4, ["B${^+}$-Tree${_{NH}}$", "B${^+}$-Tree${_{CA}}$", "FPTree", "B${^+}$-Tree${_{C}}$"], memtype)
        # plot_graph('scaling/' + memtype + '/' + result_file_dirs_1[i], result_file_dir1, result_file_dir2, result_file_dir4, result_file_dir3, [r'$\rm{B^{+}}$-$\rm{Tree_{NH}}$', r"$\rm{B^{+}}$-Tree$\rm{_{NH(DRAM)}}$", r"$\rm{B^{+}}$-Tree", "FPTree"], memtype)
        # plot_graph('scaling/' + memtype + '/' + result_file_dirs_1[i], result_file_dir1, result_file_dir2, result_file_dir4, result_file_dir3, ['NV-HTM', "DRAM", r"$\rm{B^+}$-木", "FPTree"], memtype)
    plot_graph('scaling/' + memtype + '/', result_file_dir1, result_file_dir2, 0, 0, ['ログ圧縮', "通常", r"", ""], memtype)
        # plot_graph('scaling/' + memtype + '/' + result_file_dirs_1[i], result_file_dir1, result_file_dir2, result_file_dir4, result_file_dir3, ['NV-HTM(NVM)', "NV-HTM(DRAM)", r"HTM(DRAM)", "FPTree"], memtype)
