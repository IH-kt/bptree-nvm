...
fontsize = 18
labels = ['初期化', 'ログエントリの選択', 'ログの読み出し', 'ライブラリの使用', 'ログの書き出し', 'ログの反映と永続化']
...
for logsize in logsizes:
    for op in ops:
        ...
        fls_bottom_p = apl_bottom_p + pmem_file['logwrite']
        top_p = fls_bottom_p + pmem_file['apply-flush']
        ...
        plt.bar(xtick, pmem_file['logwrite'], bottom=apl_bottom_p, width=barwidth, edgecolor = 'k', color = colorlst[4], label = labels[4])
        plt.bar(xtick, pmem_file['apply-flush'], bottom=fls_bottom_p, width=barwidth, edgecolor = 'k', color = colorlst[5], label = labels[5])
