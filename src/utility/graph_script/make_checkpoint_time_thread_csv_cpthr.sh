...
do
    ...
    do
        ...
        do
            echo 'thread,reserve,commit-finding,log-read,library,apply,flush' > ${result_dir}/checkpoint_${op}_${type}.logsize.${logsz}.csv
            ...
            do
                ...
                do
                    ...
                    write_time=`cat ${target_dir}/${op}_concurrent.exe.thr.${thr}.trial.${trial}.dmp | grep "d wr" | cut -f 5 -d ' '`
                    ...
                    flush_time=`cat ${target_dir}/${op}_concurrent.exe.thr.${thr}.trial.${trial}.dmp | grep "d flush" | cut -f 5 -d ' '`
                    ...
                    write_time_sum=`echo "scale=7; ${write_time} + ${write_time_sum}" | bc`
                    ...
                    flush_time_sum=`echo "scale=7; ${flush_time} + ${flush_time_sum}" | bc`
                done
                rsrv_time_avg=`echo "scale=7; (${reserve_time_sum} / ${max_trial})" | bc`
                cmfd_time_avg=`echo "scale=7; (${commit_find_time_sum} / ${max_trial})" | bc`
                writ_time_avg=`echo "scale=7; (${write_time_sum} / ${max_trial})" | bc`
                read_time_avg=`echo "scale=7; (${read_only_time_sum} / ${max_trial})" | bc`
                nowr_time_avg=`echo "scale=7; (${no_write_time_sum} / ${max_trial})" | bc`
                flsh_time_avg=`echo "scale=7; (${flush_time_sum} / ${max_trial})" | bc`
                write_time_diff=`echo "scale=7; ${writ_time_avg} - ${nowr_time_avg}" | bc`
                libry_time_diff=`echo "scale=7; ${nowr_time_avg} - ${read_time_avg}" | bc`
                echo "$thr,$rsrv_time_avg,$cmfd_time_avg,$read_time_avg,$libry_time_diff,$write_time_diff,$flsh_time_avg" >> ${result_dir}/checkpoint_${op}_${type}.logsize.${logsz}.csv
            done
        done
    done
done
