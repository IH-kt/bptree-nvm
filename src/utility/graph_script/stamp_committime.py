...
def commit_plot(target_dir, data, bench_name, labels, colorlst, font_size):
    ...
    plt.tight_layout()
    plt.savefig(target_dir + '/commit_' + bench_name + '.pdf')
    plt.close()

def plot_graph(target_dir, data, bench_name, labels, colorlst, font_size):
    os.makedirs(target_dir, exist_ok=True)
    commit_plot(target_dir, data, bench_name, labels, colorlst, font_size)
...
def main():
    ...
    # tree_types = ['parallel_cp', 'parallel_cp_dram_log', 'no_cp']
    # tree_types = ['optimized_commit', 'no_cp_optc']
    etypes = ['use_mmap', 'use_mmap_dram', 'log_comp', 'no_cp', 'no_cp_dram']
    ...
    for etype in etypes:
        for bench_name in bench_names:
            ...
            dataframe = pd.read_csv('committime/' + etype + '/commit_' + bench_name + '.csv', index_col=0)
            plot_graph('committime/' + etype, dataframe, bench_name, labels, colorlst, font_size)
