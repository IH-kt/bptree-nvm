...
def plot_graph(plot_infos):
    ...
    for df in result_dataframes:
        df['Time_remain'] = df['Time']
        for tc in plot_infos['time_class']:
            df['Time_remain'] -= df[tc]
    time_class = copy.copy(plot_infos['time_class'])
    time_class.append('Time_remain')
    ...
    for df in result_dataframes:
        ...
        bar_bottom = 0
        for (color, tc, hatch) in zip(plot_infos['colorlst'], time_class, plot_infos['hatches']):
            ...
            bar_bottom += df[tc]
        top_of_bar = max([max(df['Time']), top_of_bar])
        x_position += 1
...
def compare_logcomp(plot_infos):
    ...
    # plot_infos['time_class'] = ['Checkpoint-block', 'Abort']
    plot_infos['time_class'] = ['Checkpoint-block', 'Abort', 'HTM-block']
...
def main():
    ...
    plot_infos['fn_generator'] = lambda bench_name: lambda extype: 'waittime/' + extype + '/waitall_' + bench_name + '.csv'
    plot_infos['colorlst'] = ['#000000', '#999999', '#bbbbbb', '#ffffff']

    plot_infos['plot_target_dir'] = "graphs/waittime"
    # labels = ['Checkpointプロセスによるログ処理の待ち時間', 'トランザクションのアボートによる消費時間', 'フォールバックによる待ち時間']
    plot_infos['labels'] = ['Checkpointプロセスによるログ処理の待ち時間', 'トランザクションのアボートによる消費時間', 'フォールバックによる待ち時間', '本来の処理にかかった時間']
