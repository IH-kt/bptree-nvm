...
function make_csv () {
    ...
    echo 'thread,reserve,commit-finding,logwrite,apply-log-flush' > ${target_dir}/checkpointtime_${bench_type}.csv
    ...
    do
        ...
        do
            ...
            lcheck_time=`cat ${source_dir_tmp}/result_thr${thr}_trial${trial} | grep "d log" | cut -f 5 -d ' '`
            acheck_time=`cat ${source_dir_tmp}/result_thr${thr}_trial${trial} | grep "d wri" | cut -f 7 -d ' '`
            rcheck_time_sum=`echo "scale=7; ${rcheck_time} + ${rcheck_time_sum}" | bc`
            ccheck_time_sum=`echo "scale=7; ${ccheck_time} + ${ccheck_time_sum}" | bc`
            lcheck_time_sum=`echo "scale=7; ${lcheck_time} + ${lcheck_time_sum}" | bc`
        ...
        lcheck_time_avg=`echo "scale=7; (${lcheck_time_sum} / ${max_trial})" | bc`
        acheck_time_avg=`echo "scale=7; (${acheck_time_sum} / ${max_trial})" | bc`
        echo "$thr,$rcheck_time_avg,$ccheck_time_avg,$lcheck_time_avg,$acheck_time_avg" >> ${target_dir}/checkpointtime_${bench_type}.csv
    done
}
