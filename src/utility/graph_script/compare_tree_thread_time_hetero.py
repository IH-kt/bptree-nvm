...
def plot_graph(log_size_str, result_file1, result_file2, result_file3, result_file4, cols, memtype):
    ...
    for i in range(3):
        ...
        for (col, color, marker) in zip(result_df, colorlst, markerlst):
            ...
            tmp.plot(ax=ax, color=color, marker=marker, figsize=(6,5))
        ...
        plt.legend(cols, bbox_to_anchor=(0.40, -0.40), loc='center', borderaxespad=0, ncol=2, fontsize=font_size)
        plt.tick_params(labelsize=font_size)
        # plt.title('スレッド数による実行時間の変化 (' + ops[i] + ')', fontsize=font_size)
        plt.tight_layout()
        # plt.savefig(log_size_str + '/' + colname + '.png')
        # plt.savefig(log_size_str + '/' + colname.split('_concurrent.exe')[0] + '_ja.pdf')
...
for memtype in ['pmem']: # ['vmem']: # ['pmem', 'vmem']:
    result_file_dirs_1 = get_logsz_dir('../' + memtype + '/elapsed_time/bptree_nvhtm_log_pmem/')
    # result_file_dirs_2 = get_logsz_dir('../' + memtype + '/elapsed_time/bptree_nvhtm_1/')
    result_file_dirs_2 = get_logsz_dir('../' + memtype + '/elapsed_time/bptree_nvhtm_usr_pmem/')
    result_file_dirs_3 = get_logsz_dir('../' + memtype + '/elapsed_time/bptree_nvhtm_plain/')
    result_file_dirs_4 = get_logsz_dir('../' + 'vmem' + '/elapsed_time/bptree_nvhtm_plain_vmem/')

    for i in range(len(result_file_dirs_1)):
        result_file_dir1 = '../' + memtype + '/elapsed_time/bptree_nvhtm_log_pmem/' + result_file_dirs_1[i]
        # result_file_dir2 = '../' + memtype + '/elapsed_time/bptree_nvhtm_1/' + result_file_dirs_2[i]
        result_file_dir2 = '../' + memtype + '/elapsed_time/bptree_nvhtm_usr_pmem/' + result_file_dirs_2[i]
        result_file_dir3 = '../' + memtype + '/elapsed_time/bptree_nvhtm_plain/' + result_file_dirs_2[i]
        # result_file_dir3 = '../' + memtype + '/elapsed_time/fptree_concurrent_0/' + result_file_dirs_3[0]
        result_file_dir4 = '../' + 'vmem' + '/elapsed_time/bptree_nvhtm_plain_vmem/' + result_file_dirs_4[0]
        # plot_graph('scaling/' + memtype + '/' + result_file_dirs_1[i], result_file_dir1, result_file_dir2, result_file_dir3, result_file_dir4, ["ログのみ", "ユーザデータのみ", "両方NVM", "両方DRAM"], memtype)
        # plot_graph('scaling/' + memtype + '/' + result_file_dirs_1[i], result_file_dir1, result_file_dir2, result_file_dir3, result_file_dir4, ["Only log", "Only leaf node", "Both", "Neither"], memtype)
        plot_graph('scaling/' + memtype + '/' + result_file_dirs_1[i], result_file_dir1, result_file_dir2, result_file_dir3, result_file_dir4, ["Only log", "Only user data", "Both", "Neither"], memtype)
