...
def plot_graph(log_size_str, result_file1, result_file2, result_file3, result_file4, cols, memtype):
    ...
    for i in range(3):
        ...
        result_df = pd.concat([p_df1, p_df3], axis=1)
        result_df.columns = ['col1', 'col3']
        # result_df = p_df1
        ...
        for (col, color, marker) in zip(result_df, colorlst, markerlst):
            ...
            tmp.plot(ax=ax, color=color, marker=marker, figsize=(5,3))
        ...
        # plt.legend(cols, bbox_to_anchor=(0.40, -0.10), loc='center', ncol=2, borderaxespad=0, fontsize=font_size)
        plt.legend(cols, bbox_to_anchor=(0.35, 0.5), fontsize=font_size)
        plt.tick_params(labelsize=font_size)
        # plt.title('スレッド数による実行時間の変化 (' + ops[i] + ')', fontsize=font_size)
        plt.tight_layout()
        # plt.savefig(log_size_str + '/' + colname + '.png')
        plt.savefig(log_size_str + '/' + colname + '-part.pdf')
        plt.close()
...
for memtype in ['pmem']: # ['vmem']: # ['pmem', 'vmem']:
    ...
    for i in range(len(result_file_dirs_1)):
        ...
        plot_graph('scaling/' + memtype + '/' + result_file_dirs_1[i], result_file_dir1, 0, result_file_dir3, 0, ["B${^+}$-Tree${_{NH}}$", "FPTree"], memtype)
