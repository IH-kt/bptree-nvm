...
ca=bptree_nvhtm_1
cptype=compression
logsz_list=`ls ${root_dir}/pmem/${cptype}/${plain} | sed -e "s/logsz_//" | sort -n`
# types='plain db'
# types='plain'
types='plain'
...
do
    ...
    do
        ...
        mkdir -p abort/$memtype/$cptype/$type_dir
        for op in $ops
        do
            echo 'type,logsize,thread,abort,conflict,capacity,explicit' > abort/$memtype/$cptype/$type_dir/abort_${op}.csv
            for logsz in $logsz_list
            do
                logsz_dir=logsz_$logsz
                target_dir=`echo "${root_dir}/${memtype}/$cptype/${type_dir}/${logsz_dir}/compressed"`
                ...
                do
                    ...
                    echo "$type,$logsz,$thr,$abort,$conflict,$capacity,$explicit" >> abort/$memtype/$cptype/$type_dir/abort_${op}.csv
                done
            done
        done
    done
