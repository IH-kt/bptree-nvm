...
mem_list = ['pmem', 'vmem']
font_size = 18
labels = ['競合', 'メモリアクセス上限超過', 'ログ容量不足', 'その他']
    ...
    for nvhtm_type in type_list:
        for i in range(0, len(op_list)):
            ...
            for logsz in log_list:
                ...
                plt.legend(labels, bbox_to_anchor=(0.50, -0.40), loc='center', borderaxespad=0, ncol=2, fontsize=font_size-4)
                plt.tight_layout()
                # plt.title('スレッド数によるアボート回数の変化（' + op_list_j[i] + '）')
                #plt.savefig('abort_' + nvhtm_type + '_' + op_list[i] + '_' + str(logsz) + '.png')
                plt.savefig('abort/' + memtype + '/' + nvhtm_type + '/total_abort_' + op_list[i] + '_logsz_' + str(logsz) + '_presen.pdf')
                plt.close()
