...
void *insert_initials(BPTree *bpt, void *arg) {
#ifdef NVHTM
    NVHTM_thr_init();
#endif
...
void *mixed_random(BPTree *bpt, void *arg) {
    arg_t *arg_cast = (arg_t *)arg;
#ifdef NVHTM
    NVHTM_thr_init();
#endif
...
int main(int argc, char *argv[]) {
    ...
    size_t allocation_size = sizeof(LeafNode) * ((initial_elements + loop_times) / (MAX_PAIR/2) * 1.01 + 3 + thread_max) + sizeof(AllocatorHeader);
#else
    // size_t allocation_size = sizeof(PersistentLeafNode) * (initial_elements / (MAX_PAIR/2) + 2 + thread_max) + sizeof(AllocatorHeader);
    size_t allocation_size = sizeof(PersistentLeafNode) * ((initial_elements + loop_times) / (MAX_PAIR/2) + 3 + thread_max) + sizeof(AllocatorHeader);
#endif
...
#endif
    random_init(initial_elements, loop_times, thread_max);
    bpt = newBPTree();
...
// #endif
    ...
    arg[thread_max] = (arg_t *)malloc(sizeof(arg_t));
    arg[thread_max]->seed = 0;
    arg[thread_max]->tid = thread_max;
    arg[thread_max]->loop = initial_elements;
    tid_array[thread_max] = bptreeCreateThread(bpt, insert_initials, warmup, arg[thread_max]);
    bptreeWaitThread(tid_array[thread_max], NULL);
    free(arg[thread_max]);
    ...
    for (i = 0; i < thread_max-1; i++) {
        ...
        tid_array[i] = bptreeCreateThread(bpt, mixed_random, warmup, arg[i]);
    }
    arg[i] = (arg_t *)malloc(sizeof(int));
    arg[i]->seed = i;
    arg[i]->tid = i % 256 + 1;
